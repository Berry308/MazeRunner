...
class UMRExperienceManagerComponent;
class UObject;
struct FFrame;
...
class AMRGameState : public AModularGameStateBase, public IAbilitySystemInterface
{
	GENERATED_BODY()

public:

	 AMRGameState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	// void MulticastMessageToClients(const FMRVerbMessage Message);

	// Send a message that all clients will be guaranteed to get
	// (use only for client notifications that cannot handle being lost)
	//UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "Lyra|GameState")
	// void MulticastReliableMessageToClients(const FMRVerbMessage Message);
...
private:
	...
	UPROPERTY()
	TObjectPtr<UMRExperienceManagerComponent> ExperienceManagerComponent;

	// The ability system component subobject for game-wide things (primarily gameplay cues)
	UPROPERTY(VisibleAnywhere, Category = "MazeRunner|GameState")
