...
#include "MaruWeaponStateComponent.generated.h"
...
// Hit markers are shown for ranged weapon(远程武器) impacts in the reticle
// A 'successful' hit marker is shown for impacts that damaged an enemy
struct FMaruScreenSpaceHitLocation
{
...
struct FLyraServerSideHitMarkerBatch
{
	...
	TArray<FMaruScreenSpaceHitLocation> Markers;
...
UCLASS()
class UMaruWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UMaruWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FMaruScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FMaruScreenSpaceHitLocation> LastWeaponDamageScreenLocations;
