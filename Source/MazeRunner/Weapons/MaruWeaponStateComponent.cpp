...
#include "MaruWeaponStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MaruWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UMaruWeaponStateComponent::UMaruWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UMaruWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

	if (APawn* Pawn = GetPawn<APawn>())
	{
		if (UMaruEquipmentManagerComponent* EquipmentManager = Pawn->FindComponentByClass<UMaruEquipmentManagerComponent>())
		{
			if (UMaruRangedWeaponInstance* CurrentWeapon = Cast<UMaruRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UMaruRangedWeaponInstance::StaticClass())))
			{
...
bool UMaruWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
...
bool UMaruWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UMaruWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		...
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FMaruScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UMaruWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FMaruScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UMaruWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UMaruWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UMaruWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
