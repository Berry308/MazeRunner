...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MaruWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

UMaruWeaponInstance::UMaruWeaponInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (UMRHealthComponent* HealthComponent = UMRHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void UMaruWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UMaruWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void UMaruWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float UMaruWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
//TSubclassOf<UAnimInstance> UMaruWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
//{
//	const FLyraAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
//	return SetToQuery.SelectBestLayer(CosmeticTags);
//}

const FPlatformUserId UMaruWeaponInstance::GetOwningUserId() const
{
...
void UMaruWeaponInstance::ApplyDeviceProperties()
{
...
void UMaruWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void UMaruWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
