...
#include "MaruRangedWeaponInstance.h"
#include "NativeGameplayTags.h"
#include "GameFramework/Pawn.h"
#include "GameFramework/CharacterMovementComponent.h"
//#include "Camera/LyraCameraComponent.h"
//#include "Physics/PhysicalMaterialWithTags.h"
#include "Weapons/MaruWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(MaruRangedWeaponInstance)

//UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Lyra_Weapon_SteadyAimingCamera, "Lyra.Weapon.SteadyAimingCamera");

UMaruRangedWeaponInstance::UMaruRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UMaruRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UMaruRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UMaruRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UMaruRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UMaruRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UMaruRangedWeaponInstance::Tick(float DeltaSeconds)
{
	//这里的Tick没有每帧执行，要被WeaponStateComponent中的Tick调用执行
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UMaruRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UMaruRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UMaruRangedWeaponInstance::AddSpread()
{
...
float UMaruRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UMaruRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UMaruRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UMaruRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
